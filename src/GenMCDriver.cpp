    ...
    exploredBlocked(0), duplicates(0), start(start)
{
  ...
  WARN("ENTERING GENMC CONSTRUCTOR! \n");
  ModuleInfo MI;
  std::string buf;

  // EDITED: NO NEED TO PREPARE ANY MODULE
  ...
  // EDITED: NO NEED TO CREATE ANY INTERPRETER
  ...
  WARN("EXITING GENMC CONSTRUCTOR! \n");
}
...
void GenMCDriver::run()
{
  ...
  WARN("ENTERING THE RUN FUNCTION OF THE DRIVER! \n");
  explore();
  printResults();
  return;
}
...
void GenMCDriver::explore()
{
  ...
  while (true) {
    ...
    /*
      TODO: Interpret the system call events!
    */
